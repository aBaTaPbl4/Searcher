...
namespace SearcherTests.Models
{
    ...
    public class SearchProcessTest
    {
        ...
        {
            Assert.AreEqual(TestHelper.FilesInFirstTestDir, _search.FilesProcessed, "Wrong processed files number. {0}",
                            Log.Content);
        ...
        [Test,
         NUnit.Framework.Description(
             "Если не подписались на события поиска, то все равно поиск должен успешно проходить")]
